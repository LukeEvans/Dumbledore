...
object Itunes {
  ...
  def getTopSongs(limit: Int):ListBuffer[Object] = {
    
    val response = Tools.fetchURL(baseRSSURL + "topsongs/limit=" + limit + "/json")
    
    if(response.get("feed").has("entry"))
      return extractData(response.get("feed").get("entry"), (new ItunesSong(_)))
    else
      return ListBuffer[Object]()
  }
  
  
  private def extractData(dataNode:JsonNode, constructor:(JsonNode) => Object):ListBuffer[Object] = {
...
class ItunesSong{
  ...
  var rating:String = null
  var date:String = null
  var story_type:String = "itunes_song"
