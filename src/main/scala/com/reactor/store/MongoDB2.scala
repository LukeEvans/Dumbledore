...
class MongoDB2(collectionName:String) {
	 ...
	 */
	def find(queryObj:DBObject, limit:Int):ListBuffer[Object] = {

	  val cursor = collection.find(queryObj).limit(limit).sort(MongoDBObject("date" -> -1))
	  
	  val list = collection.find(queryObj).limit(limit).sort(MongoDBObject("date" -> -1)).toArray()
